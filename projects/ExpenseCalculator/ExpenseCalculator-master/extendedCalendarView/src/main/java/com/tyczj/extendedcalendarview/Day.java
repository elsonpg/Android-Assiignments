	...
	private class GetEvents extends AsyncTask<Void,Void,Void>{
		...
		protected Void doInBackground(Void... params) {
			Cursor c = context.getContentResolver().query(CalendarProvider.CONTENT_URI,new String[] {CalendarProvider.ID,CalendarProvider.EVENT,
					CalendarProvider.DESCRIPTION,CalendarProvider.LOCATION,CalendarProvider.START,CalendarProvider.END,CalendarProvider.COLOR,CalendarProvider.YESCOUNT, CalendarProvider.NOCOUNT, CalendarProvider.MAYBECOUNT, CalendarProvider.MYSTATUS, CalendarProvider.POSTAUTHOR},"?>="+CalendarProvider.START_DAY+" AND "+ CalendarProvider.END_DAY+">=?",
			...
			if(c != null && c.moveToFirst()){
				do{
					...
					event.setYescount(c.getString(7));
					event.setNocount(c.getString(8));
					event.setMaybecount(c.getString(9));
					event.setMystatus(c.getString(10));
					event.setPostauthor(c.getString(11));
					events.add(event);
				}while(c.moveToNext());	
				c.close();
			}
